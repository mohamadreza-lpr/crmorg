<template>
    <div>
        <b-card>
            <h4 class="mb-3">ویرایش قرعه کشی</h4>
            ...
            <b-form @submit.prevent="updatePosts(id)">
                <b-row>
                    <b-col cols="12">
                        <b-form-group
                            label="نام"
...
export default {
    ...
    methods: {
        getPosts(id) {
            axios
                .get(`${process.env.VUE_APP_URL}/lotteries/${id}`, {
                    headers: {
                ...
                .then(response => {
                    ...
                    this.starts_at = response.data.data.starts_at;
                    this.expires_at = response.data.data.expires_at;
                    this.execute_at = response.data.data.full_execute_at_date;
                    // this.type = response.data.data.type;
                    this.max_winner = response.data.data.max_winner;
                    this.max_chances = response.data.data.max_chances;
                    this.required_points = response.data.data.required_points;
                    this.point_withdrawal = response.data.data.point_withdrawal;
                    this.status = _.find(this.status_options, item => {
                        return item.text === response.data.data.status;
                    }).value;
                    this.description = response.data.data.description.content;
                })
        ...
        updatePosts(id) {
            ...
            axios
                .patch(
                    `${process.env.VUE_APP_URL}/lotteries/${id}/update`,
                    params,
                    {
                        headers: {
                            Authorization: `Bearer ${localStorage.getItem(
