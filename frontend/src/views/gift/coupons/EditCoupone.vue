<template>
    <div>
        ...
        <BCard>
            <h4 class="mb-2 font-weight-bolder">ویرایش کوپن</h4>
            <div class="row">
            ...
            <div class="text-right">
                <b-button variant="primary" @click="updateCouponHandler">
...
export default {
    ...
    methods: {
        ...
        getCoupon(id) {
            axios
                .get(`${process.env.VUE_APP_URL}/discounts/${id}/show`, {
                    headers: {
                ...
                .then(({ data }) => {
                    this.couponTitle = data.data.title;
                    this.couponType = _.find(this.couponTypeOptions, [
                        "name",
                        data.data.type,
                    ]);
                    this.couponPerUser = data.data.per_user;
                    this.couponCode = data.data.code;
                    this.couponMaxUsage = data.data.max_usage;
                    this.couponAmount = data.data.amount;
                    this.couponImagePreview = data.data.thumb_img;
                    this.point = data.data.required_points;
                    this.imageSelected = data.data.thumb_img ? true : false;
                    this.couponBillMinAmount = data.data.bill_min_amount;
                    this.couponStartedAt = data.data.starts_at;
                    this.couponExpiresAt = data.data.expires_at;
                    this.couponStatus =
                        data.data.status == "فعال" ? "active" : "inactive";
                });
        },
        // update the coupon
        updateCouponHandler() {
            ...
            axios
                .post(
                    `${process.env.VUE_APP_URL}/discounts/${id}/update`,
                    formData,
                    {
                        headers: {
                            Authorization: `Bearer ${localStorage.getItem(
                    ...
                    this.$router.replace("/gift/coupones/list").then(() => {
                        this.$toast({
                            ...
                            props: {
                                ...
                                icon: "CoffeeIcon",
                                variant: "success",
                                text: "کوپن مورد نظر بروزرسانی شد",
                            },
                        });
                    });
                })
