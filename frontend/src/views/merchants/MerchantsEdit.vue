    ...
    <div>
        ...
        <b-row>
            <b-col md="12" lg="12" sm="12">
                <BCard>
                    <h4 class="mb-2 font-weight-bolder">ویرایش شعبه</h4>
                    <div class="row">
...
export default {
    ...
    mounted() {
        ...
        axios
            .get(`${process.env.VUE_APP_URL}/branches/${id}/show`, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem(
                        "accessToken"
                    )}`
            ...
            .then(response => {
                ...
                this.branch.userIds = _.map(response.data.data.users, item => {
                    return {
                        ...item,
                        name:
                            item.first_name +
                ...
                this.branch.userIds = _.map(this.branch.userIds, item => {
                    return item.name;
                });
                this.branch.name = response.data.data.name;
                this.branch.code = response.data.data.code;
                this.branch.status =
                    response.data.data.status == "فعال" ? "active" : "inactive";
                this.branch.address = response.data.data.address.line;
                this.branch.postcode = response.data.data.address.postcode;
                this.branch.province = response.data.data.province;
                this.provinceChanges();
                this.branch.city = response.data.data.city;
                this.branch.group = response.data.data.groups[0];
                // this.branch.  =
                // this.branch.seller = {
                ...
                // };
                this.telephones = response.data.data.phones;
                // setting on vue
                this.$store.commit(
                    "app/setTelephones",
                    response.data.data.phones
                );
            });
        // get provinces
        axios
        ...
        axios
            .get(
                `${process.env.VUE_APP_URL}/users?role=seller&paginate=false`,
                {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem(
                            "accessToken"
    ...
    methods: {
        ...
        updateMerchantHandler() {
            ...
            const params = {
                ...
                user_id: this.branch.seller ? this.branch.seller.id : undefined,
            ...
            axios
                .patch(
                    `${process.env.VUE_APP_URL}/branches/${id}/update`,
                    params,
                    {
                        headers: {
                            Authorization: `Bearer ${localStorage.getItem(
